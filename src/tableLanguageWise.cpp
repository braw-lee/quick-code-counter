...
TableLanguageWise::TableLanguageWise(const std::map<LanguageId, FileCountInfo>& finalData)
...
void TableLanguageWise::print()
{
	...
	for(auto& it : _finalData)
	{
		std::array<std::string, +column_size> cellString {
			...
			std::to_string(100.0 * it.second._lineInfo.total / _finalData.at(LanguageId::total)._lineInfo.total)
		};
...
void TableLanguageWise::setCellSize()
{
	...
	for(auto& it : _finalData)
	{
		...
		_cellSize[+ratio] = std::max(_cellSize[+ratio], std::to_string(100.0 * it.second._lineInfo.total / _finalData.at(LanguageId::total)._lineInfo.total).size());
	}
}
...
void TableLanguageWise::printColumn(std::array<std::string, +column_size>& column)
{
	std::cout<<'\n';
